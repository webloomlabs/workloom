-- Fixed engagement fees join the project's cost.
--
-- A member on a fixed fee costs the project a lump sum rather than an hourly
-- rate. Their time entries are snapshotted at a cost rate of zero (see
-- `applyFixedFee`), so the hours stay visible without being paid for twice, and
-- the fee itself is added here instead.
--
-- `labour_cost_minor` keeps meaning *hourly* labour. The two sit side by side
-- rather than one absorbing the other, because "we paid four thousand for this
-- person" and "they cost us ninety an hour" are different facts about a project
-- and a margin that blends them cannot be explained to anyone.
--
-- CREATE OR REPLACE requires the existing columns keep their names, types and
-- order; the two new ones are appended. `margin_minor` changes expression but
-- not position, which is allowed.
--> statement-breakpoint
CREATE OR REPLACE VIEW project_financials_v WITH (security_invoker = true) AS
...
fixed AS (
  -- The fee has no currency of its own: it is denominated in the project's, the
  -- same as the hourly overrides beside it. So the currency comes from the
  -- project, which also keeps this CTE's key shape identical to the others'.
  SELECT pm.organization_id, pm.project_id, p.currency,
         sum(pm.fixed_fee_minor)::bigint AS fixed_cost_minor,
         count(*)::int                   AS members_with_fixed_fee
  FROM project_members pm
  JOIN projects p ON p.organization_id = pm.organization_id AND p.id = pm.project_id
  WHERE pm.fixed_fee_minor IS NOT NULL
  GROUP BY pm.organization_id, pm.project_id, p.currency
),
keys AS (
  ...
  UNION
  -- So a project whose only cost so far is a fixed fee still reports a row.
  SELECT organization_id, project_id, currency FROM fixed
)
SELECT
  k.organization_id,
  k.project_id,
  ...
  (coalesce(b.billed_minor, 0)
     ...
     - coalesce(s.expense_cost_minor, 0)
     - coalesce(f.fixed_cost_minor, 0))::bigint AS margin_minor,
  ...
  coalesce(f.fixed_cost_minor, 0)::bigint      AS fixed_cost_minor,
  coalesce(f.members_with_fixed_fee, 0)::int   AS members_with_fixed_fee
FROM keys k
...
LEFT JOIN fixed  f ON f.organization_id = k.organization_id AND f.project_id = k.project_id AND f.currency = k.currency;
